...
function loadCards(cardSpace){
	...
	for(var key in cardList){
		...
		cardSpace.append('<div id="card' + loopy + '" class="card"><span class="cardtitle">' + key + '</span></div>');

		// Load the entire text of the cards as invisible divs so we can make them into dialogs later.
		$('#hiddenspace').append('<div id="bigcard' + loopy + '" class="bigtextcard"><b>' + key + '</b><p>' + cardList[key].text + '</p></div>');
...
function loadGroups(groupSpace, cardSpace, options){
	...
	if(options == 'all'){
		...
		for(var key in groupList){
			groupSpace.append('<div id="group' + loopy + '" class="group"><p class="title">' + key + '</p></div>');
	...
	// 'blank' adds a single blank group with number higher than any extant group.
		...
		if(groupCount < (cardCount-1)){
			...
			groupSpace.append('<div id="group' + newGroupNum + '" class="group"><p class="title">Namnlös ' + newGroupNum + '</p><br/></div>');
...
function nameChanger(title, text, groupNumber){
   ...
   $(title).replaceWith('<textarea id="grouprename" rows="1" cols="30">' + text + '</textarea>');
...
//		console.log(JSProblemState['pairings'][i],card.attr('id'));
//		if(JSProblemState['pairings'][i].match(card.attr('id'))){ // BUGG!!!!
		if(JSProblemState['pairings'][i].endsWith(card.attr('id'))){ // BUGG!!!!
...
function output() {
//	console.log(JSProblemState['pairings'].length,Object.keys(JSProblemState['cards']).length);
//	console.log(JSProblemState['pairings']);
	if(JSProblemState['pairings'].length!=Object.keys(JSProblemState['cards']).length){
		showMessage("Du är inte klar :-(<br>Alla kort ska flyttas till en kategori.<br><span style='float:right'><button type='button' onclick='closeMessage()'><b>OK</b></button></span>",300); return 1;}
	if($('#nickName').val().length<1) {
		showMessage("Du är inte angett namn eller nick :-(<br><span style='float:right'><button type='button' onclick='closeMessage()'><b>OK</b></button></span>",300); return 1;}
	showMessage("Klart! Tack för hjälpen!",300);
	var text = "Sent by: "+$('#nickName').val();
	for(var g = 1; g < Object.keys(JSProblemState['groups']).length; g++) {
		text+='\r\n'+JSProblemState['groups'][g]+':';
		for(var i = 1; i < JSProblemState['pairings'].length; i++)
			if(JSProblemState['pairings'][i].substring(0, 6)==("group"+g)) text+=' '+JSProblemState['cards'][JSProblemState['pairings'][i].substring(12,JSProblemState['pairings'][i].length)];
	}
	text+='\r\n\r\n'
	console.log(text);
	
	var xhr = new XMLHttpRequest();
	var params = 'cards='+encodeURIComponent(text);
	xhr.open("POST", 'save_cards.php', true);
